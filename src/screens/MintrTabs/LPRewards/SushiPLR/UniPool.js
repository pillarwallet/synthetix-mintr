...
const UniPool = ({ goBack, walletDetails }) => {
	...
	const fetchAllowance = useCallback(async () => {
		...
		try {
			setIsLoading(true);
			const allowance = await sushiContract.allowance(currentWallet, sushiPLRETHContract.address);
	...
	useEffect(() => {
		...
		sushiContract.on('Approval', (owner, spender) => {
			if (owner === currentWallet && spender === sushiPLRETHContract.address) {
				setAllowance(true);
			}
		});
		...
		return () => {
			if (snxJSConnector.initialized) {
				sushiContract.removeAllListeners('Approval');
			}
		};
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [currentWallet]);
